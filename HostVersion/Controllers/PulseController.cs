...
namespace HostVersion.Controllers
{
    ...
    {
        [Route("~/api/pulse/notify_bot_view_resized")]
        [HttpPost]
        public async Task<ActionResult<Packet>> NotifyBotViewResized([FromBody] Packet packet)
        {
            using (var dbContext = new DatabaseContext())
            {
                ...
                var result = await new KafkaTransport().AskPairedPeer<BotViewResizedRequest, BotViewResizedResponse>(
                    new BotViewResizedRequest()
                    {
                        Packet = packet,

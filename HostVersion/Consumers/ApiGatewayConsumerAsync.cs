...
namespace HostVersion.Consumers
{
    public class ApiGatewayConsumerAsync : IConsumerAsync<ComplexDeletionPush>, IConsumerAsync<RoomDeletionPush>, IConsumerAsync<ContactCreationPush>
        ...
        , IConsumerAsync<AppCreatedNotif>, IConsumerAsync<UserRequestedBotPreviewPush>, IConsumerAsync<BotLoadedPush>
        , IConsumer<BotViewResizedPush>
    {
            ...
            using (var dbContext = new DatabaseContext())
            {
                foreach (var sessionId in context.Message.SessionIds)
                {
                    ...
                    var notification = new UserRequestedBotViewNotification()
                    {
                        ...
                        BotWindowMode = context.Message.Notif.BotWindowMode,
                        SessionId = session.SessionId
                    };
        ...
        public async Task Consume(ConsumeContext<BotLoadedPush> context)
        {
            using (var dbContext = new DatabaseContext())
            {
                foreach (var sessionId in context.Message.SessionIds)
                {
                    ...
                    var notification = new BotLoadedNotification()
                    {
                        RoomId = context.Message.Notif.RoomId,
                        ComplexId = context.Message.Notif.ComplexId,
                    ...
                    using (var mongo = new MongoLayer())
                    {
                        notification.Type = notification.GetType().Name;
                        var json = JsonConvert.SerializeObject(notification);
                        var obj = JsonConvert.DeserializeObject<BotLoadedNotification>(json);
                        await mongo.GetNotifsColl2().InsertOneAsync(obj);
                    }
                }
        ...
        public async void Consume(ConsumeContext<BotViewResizedPush> context)
        {
            using (var dbContext = new DatabaseContext())
            {
                foreach (var sessionId in context.Message.SessionIds)
                {
                    ...
                    var notification = new BotViewResizedNotification()
                    {
                        UserId = context.Message.Notif.UserId,
                        ComplexId = context.Message.Notif.ComplexId,
                        RoomId = context.Message.Notif.RoomId,
                        NewWidth = context.Message.Notif.NewWidth,
                        NewHeight = context.Message.Notif.NewHeight,
                        SessionId = session.SessionId
                    };
                    ...
                    notification.Type = notification.GetType().Name;
                    var json = JsonConvert.SerializeObject(notification);
                    var obj = JsonConvert.DeserializeObject<BotViewResizedNotification>(json);
                    await mongo.GetNotifsColl2().InsertOneAsync(obj);
                }
